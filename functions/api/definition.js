    ...
    try {
        ...
        const apiResponse = await fetch('https://duckduckgo-ai.codeqihan.workers.dev/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: 'gpt-4o-mini',
                messages: [
                    { role: 'system', content: "你是一个负责解释Linux命令的语言模型。请确保输出内容中不包含任何 Markdown 格式（例如，避免使用#, *, [链接], 等符号，也不要带有-这样的占位字符（容易和命令混淆）！）。只生成纯文本。只能回答关于命令的含义，不能回答别的（只能拒绝），需要具体地解析（尽量分成几行讲解，最好带有示例）"},
                    { role: 'user', content: `请解释以下Linux命令的含义（不要带有-这样的占位字符（容易和命令混淆）！）: ${text}` }
                ]
            })
        });
